...
def main(experiment_name, is_continue_execution, final_vocab_size, \
	...
	models = Unigram.Unigram_Models(experiment_name, is_continue_execution, final_vocab_size, initial_vocab_size, partial_corpus_filepath)
	Unigram_model = models.get_Unigram_model()
	...
	if not is_continue_execution:
		wp_trainer = Embeddings.EmbeddingsTrainer(Unigram_model, corpus, window_size, log)
	...
	Unigram_model_object = Unigram.Model(experiment_name, log, Unigram_model, "Unigram", \
		target_embeddings, context_embeddings, partial_corpus, max_lines_per_token, window_size, is_continue_execution, vocab_filepath)
	bpe_vanilla_model_object = Unigram.Model(experiment_name, log, bpe_vanilla_model, "bpe_vanilla", \
	...
	current_vocab = Unigram_model_object.get_current_vocab()
	...
	if is_continue_execution and exists(sorted_tokens_sg_filepath):
		...
		Unigram_model_object.log_experiments_model_results(partial_corpus_filepath)
		bpe_vanilla_model_object.log_experiments_model_results(partial_corpus_filepath)

		# And Log starting-point SG objective
		total_skipgram_ns_probability = Unigram_model_object.total_sg_log_prob(partial_corpus_filepath)
		...
		original_token_and_sg_log_prob_without_it = Unigram_model_object.get_sg_log_prob_without_tokens_mp(total_skipgram_ns_probability, partial_corpus_filepath, True)
	...
	current_vocab = Unigram_model_object.get_current_vocab()
	current_total_sg = Unigram_model_object.total_sg_log_prob(partial_corpus_filepath)

	## prepare from ahead ####
	### now we want dict from token to index of lines - and re-encode when token is ablated
	Unigram_model_object.initialize_encoded_form_for_corpus_lines()
	Unigram_model_object.initialize_token_to_line_indices_dictionary(current_vocab, partial_corpus, experiment_name, is_continue_execution)
	...
	while len(current_vocab) > final_vocab_size:
		...
		if (iteration % iterations_until_reranking) == 0:
			token_and_sg_log_prob_without_it = Unigram_model_object.get_sg_log_prob_without_tokens_mp2(current_total_sg, True)
			...
			token_and_sg_log_prob_without_it = Unigram_model_object.get_sg_log_prob_without_tokens_mp2(current_total_sg, True, current_dict_of_top_tokens)
			sorted_tokens_sg = sorted(token_and_sg_log_prob_without_it.items(), key=lambda item: item[1])
		...
		Unigram_model_object.set_vocab(current_vocab)
		Unigram_model_object.update_encoded_form_for_corpus_lines(tokens_to_prune)
	...
	Unigram_model_object.log_experiments_model_results(partial_corpus_filepath, "Unigram")

	final_total_skipgram_ns_probability = Unigram_model_object.total_sg_log_prob(partial_corpus_filepath)
	...
	only_in_Unigram = [t for t in current_vocab if t not in bpe_vanilla_vocab]
	Unigram_only_filepath = "./results/{}/Unigram_only.txt".format(experiment_name)
	with open(Unigram_only_filepath, "w+") as Unigram_only:
		Unigram_only.write(json.dumps(only_in_Unigram, indent=4))
	...
	encoded_data = [Unigram_model.id_to_piece(x) for x in Unigram_model.encode(input_file_data)]
