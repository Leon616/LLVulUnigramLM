...
class Unigram_Models:
    def __init__(self, experiment_name, is_continue_execution, final_vocab_size, initial_vocab_size, partial_corpus_filepath):
        ...
        Unigram_model_prefix = "results/{}/Unigram".format(experiment_name)
        Unigram_model = Unigram_model_prefix + ".model"

        if not is_continue_execution:
            spm.SentencePieceTrainer.train(input=partial_corpus_filepath, model_prefix=Unigram_model_prefix, vocab_size=initial_vocab_size, model_type="bpe")
        self._Unigram_model = spm.SentencePieceProcessor(model_file=Unigram_model)
